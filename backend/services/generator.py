...
STYLES = {
    "bold_dramatic": (
        ...
        "cinematic lighting, dark moody background, and powerfull composition."
    ...
    "vibrant_energetic": (
        ...
        "composition. Then person should have an excited and engaging expression."
    )
}
...
async def process_job(job_id: str):
    ...
    with Session(engine) as session:
        ...
        # find all the thumbnails for the job
        ...
        tasks = [generate_single_thumbnail(tid, prompt, headshot_url) for tid in thumbnails_ids]
        
        # wait for all workers to finish
        await asyncio.gather(*tasks, return_exceptions=True) # *task -> this is args. unpacking, return_exceptions -> Without it: If ONE task fails; Entire gather crashes
